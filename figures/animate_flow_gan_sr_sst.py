#!/usr/bin/env python3
"""Plot and animate the Flow-SR vs GAN-SR vs Low-Res SST evaluation at full and regional scales.

Compares:
1. Low Resolution (1.6° coarse block mean input)
2. Flow Matching (0.1° Flow-SR generated SST)
3. GAN (0.1° GAN-SR generated SST)
4. Ground Truth (0.1° native OFAM target SST)

Supports generating static layout proofs (--layout-only) and rendering animated GIF/MP4 videos.
"""
...
PANELS = (
    ("sst_coarse", "flow", "Coarse input", "1.6° block mean"),
    ("sst_generated", "flow", "Flow Matching", "0.1° Flow-SR prediction"),
    ("sst_generated", "gan", "GAN Super-Resolution", "0.1° GAN-SR prediction"),
    ("sst_target", "flow", "OFAM Ground Truth", "0.1° target SST"),
)
...
class MultiModelSSTComparisonFigure:
    """4-column comparison: Coarse vs Flow-SR vs GAN-SR vs OFAM Target."""
    ...
    def __init__(
        ...
        self.ds_flow = ds_flow
        self.ds_gan = ds_gan
        self.frame = int(frame)
        self.gan_name = gan_name
        self.meshes: dict[tuple[int, str], object] = {}
        self.figure = plt.figure(figsize=(20.5, 16.2), facecolor="#f7f9fb")
        ...
        for column, (variable, source_key, heading, subtitle) in enumerate(PANELS):
            axis = self.figure.add_subplot(grid[0, column])
            values, lat, lon = self.read_field(variable, source_key, self.frame, FULL_EXTENT)
            mesh = draw_field(axis, values, lat, lon, FULL_EXTENT, labels=(column == 0))
            if column == 0:
                axis.set_xlabel("")
            self.meshes[(column, "Australia")] = mesh
            axis.set_title(
                f"{heading}\n{subtitle}",
                fontsize=11.5,
                weight="bold",
        ...
        for row, region in enumerate(REGIONS, start=1):
            for column, (variable, source_key, _, _) in enumerate(PANELS):
                axis = self.figure.add_subplot(grid[row, column])
                values, lat, lon = self.read_field(variable, source_key, self.frame, region.extent)
                mesh = draw_field(
                    axis,
                    values,
                    lat,
                ...
                if column == 0 and row < len(REGIONS):
                    axis.set_xlabel("")
                self.meshes[(column, region.name)] = mesh
        ...
        self.figure.suptitle(
            "Sea-Surface Temperature Super-Resolution Intercomparison (Low-Res vs Flow Matching vs GAN vs OFAM)",
            x=0.5,
            y=0.993,
            fontsize=16,
            weight="bold",
    ...
    def read_field(
        ...
        ds = self.ds_flow if source_key == "flow" else self.ds_gan
        lat_name, lon_name = coordinate_names(variable)
        lat = ds[lat_name].values
        lon = ds[lon_name].values
        values = ds[variable].isel(time=frame).values
        return subset_for_extent(values, lat, lon, extent)

    def update(self, frame: int):
        for column, (variable, source_key, _, _) in enumerate(PANELS):
            values, _, _ = self.read_field(variable, source_key, frame, FULL_EXTENT)
            self.meshes[(column, "Australia")].set_array(np.ma.masked_invalid(values).ravel())
            for region in REGIONS:
                values, _, _ = self.read_field(variable, source_key, frame, region.extent)
                self.meshes[(column, region.name)].set_array(np.ma.masked_invalid(values).ravel())
        timestamp = np.datetime_as_string(self.ds_flow.time.values[frame], unit="D")
        self.date_text.set_text(f"{timestamp}  |  Holdout Test Day {frame + 1} / {self.ds_flow.sizes['time']}")
        return tuple(self.meshes.values()) + (self.date_text,)


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--flow-input", type=Path, default=DEFAULT_FLOW_INPUT)
    parser.add_argument("--gan-input", type=Path, default=DEFAULT_GAN_INPUT)
    ...
    # Fallback if full_test_samples.nc or full_test_samples_ab3_pc_75step.nc
    if not arguments.flow_input.exists():
        alt = arguments.flow_input.parent / "full_test_samples_ab3_pc_75step.nc"
        if alt.exists():
            arguments.flow_input = alt

    if not arguments.gan_input.exists():
        alt = Path("/esi/project/niwa03712/rampaln/PUBLICATIONS/2026/SSTDownscaling/runs/gan_sr_v2/evaluation/full_test_samples.nc")
        if alt.exists():
            arguments.gan_input = alt

    arguments.output_dir.mkdir(parents=True, exist_ok=True)
    with xr.open_dataset(arguments.flow_input) as ds_flow, xr.open_dataset(arguments.gan_input) as ds_gan:
        if not 0 <= arguments.frame < ds_flow.sizes["time"]:
            raise ValueError(f"Frame {arguments.frame} is outside the dataset")
        
        comparison = MultiModelSSTComparisonFigure(ds_flow, ds_gan, arguments.frame)
        layout_path = arguments.output_dir / "flow_gan_sr_sst_animation_layout.png"
        ...
        frames = np.arange(0, min(ds_flow.sizes["time"], ds_gan.sizes["time"]), arguments.stride)
        if arguments.max_frames is not None:
            frames = frames[: arguments.max_frames]
        output = arguments.output or arguments.output_dir / "flow_gan_sr_sst_comparison.gif"
