#!/usr/bin/env python3
"""Plot and animate the residual-memory Flow-AR evaluation at several scales.

Uses the 364-day, truth-reset-free residual-memory rollout for 2011.
Compares:
1. Coarse input (1.6° block mean)
2. Residual-memory Flow-AR prediction (0.1° free rollout SST)
3. OFAM ground truth (0.1° native target SST)
"""
...
DEFAULT_INPUT = Path(
    ...
    "runs/flow_ar_residual_memory/evaluation/"
    "test_2011_common_noise_free_rollout_ab2pc_75step.nc"
)
...
SOURCES = (
    ...
    (
        "sst_generated",
        "Residual-memory Flow-AR",
        "0.1° truth-reset-free prediction",
    ),
...
class AutoregressiveSSTFigure:
    """Reusable artists for static layout proof and free rollout animation."""
    ...
    def __init__(
        ...
        frame: int,
        *,
        model_label: str = "Residual-memory Flow-AR",
        title: str = "Residual-Memory Flow-AR SST Super-Resolution (2011 Free Rollout)",
    ):
        self.dataset = dataset
        self.frame = int(frame)
        self.sources = (
            ("sst_coarse", "Coarse input", "1.6° block mean"),
            ("sst_generated", model_label, "0.1° truth-reset-free prediction"),
            ("sst_target", "OFAM ground truth", "0.1° target SST"),
        )
        ...
        for column, (variable, heading, subtitle) in enumerate(self.sources):
        ...
        for row, region in enumerate(REGIONS, start=1):
            for column, (variable, _, _) in enumerate(self.sources):
    ...
    def update(self, frame: int):
        for variable, _, _ in self.sources:
        ...
        self.date_text.set_text(
            f"{timestamp}  |  Lead Day {frame + 1} / {self.dataset.sizes['time']}  |  AB2/PC, 75 steps"
        )
...
def main() -> None:
    ...
    parser.add_argument("--model-label", default="Residual-memory Flow-AR")
    parser.add_argument(
        "--title",
        default="Residual-Memory Flow-AR SST Super-Resolution (2011 Free Rollout)",
    )
    parser.add_argument("--layout-name", default="flow_ar_residual_memory_sst_animation_layout.png")
    ...
    with xr.open_dataset(arguments.input) as dataset:
        ...
        comparison = AutoregressiveSSTFigure(
            dataset,
            arguments.frame,
            model_label=arguments.model_label,
            title=arguments.title,
        )
        layout_path = arguments.output_dir / arguments.layout_name
        ...
        output = (
            ...
            / "flow_ar_residual_memory_sst_comparison.gif"
        )
