...
class AccessModel:
    ...
    def historical(self) -> Path:
        return self.run_directory / (
            ...
            "historical_1980-01-01_1989-12-31_ab3pc_75step.nc"
        )
    ...
    def future(self) -> Path:
        return self.run_directory / (
            "access_cm2_converted/"
            "future_2080-01-01_2089-12-31_ab3pc_75step.nc"
        )
...
def perfect_framework_analysis() -> tuple[dict, dict[str, np.ndarray]]:
    means = stream_means(
        COMBINED_TEST,
    ...
    report = {
        ...
        "historical_skill": stream_period_skill(COMBINED_TEST, (2011, 2014)),
        "future_skill": stream_period_skill(COMBINED_TEST, (2098, 2101)),
...
def plot_access_comparison(reports: dict, fields: dict) -> Path:
    figure, axes = plt.subplots(2, len(ACCESS_MODELS), figsize=(15, 9), constrained_layout=True)
    all_signal = []
    all_error = []
    ...
    for column, model in enumerate(ACCESS_MODELS):
        record = fields[model.key]
...
def plot_access_seasons(reports: dict) -> Path:
    ...
    for offset, (model, color) in enumerate(zip(ACCESS_MODELS, colors)):
        prediction = [
...
def write_tables(report: dict) -> dict[str, str]:
    ...
    for climate, label in (("historical_skill", "Historical OFAM test"), ("future_skill", "Future OFAM test")):
        metrics = perfect[climate]
        test_rows.append({
            "Model": "Flow-SR combined historical + future",
            "Evaluation": label,
    ...
    seasonal_rows = []
    for season in SEASONS:
        metrics = perfect["seasonal_climate_signal"][season]
...
def main() -> None:
    ...
    perfect_report, perfect_fields = perfect_framework_analysis()
    access_reports = {}
    access_fields = {}
